...
このリポジトリを開いてコンテナに入り、`main.tex` を保存すれば、そのまま `out/` に PDF が出力されます。
...
このフォルダを VS Code で開くと、`.vscode/extensions.json` に基づいて推奨拡張が右下に表示されます。
...
4. コンテナの作成が終わるまで待つ（初回のみ時間がかかる）
5. `main.tex` を開いて保存する
6. `LaTeX Workshop: View LaTeX PDF` で PDF を開く
