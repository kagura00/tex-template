...
1. 基本は `Code > Download ZIP` でダウンロードして使う。GitHub リポジトリとして管理・公開したい場合のみ `Use this template` を使う
2. 取得したフォルダ（ZIP の場合は展開したフォルダ）を VS Code で開く
...
GitHub アカウントは必須ではありません。`Use this template` を使って GitHub 上で自分用リポジトリを作る場合のみ使います。

ZIP 展開後のフォルダ（または作成したリポジトリ）を VS Code で開くと、`.vscode/extensions.json` に基づいて推奨拡張の案内が表示されることがあります。
...
- 卒論・修論・投稿前原稿など、GitHub に上げたくない内容を書く場合（基本）
...
> 誤って push するのを避けたい場合は、デフォルトで `Code > Download ZIP` を使ってください。

普段の使い方は以下のとおりです。

1. GitHub から `Code > Download ZIP` を選択して取得する
2. 展開したフォルダを VS Code で開く
...
もし、テンプレートから作成したリポジトリを GitHub 上で管理したい場合は、上の 1. 2. の代わりに `Use this template` でリポジトリを作成し、それを手元でクローン、VS Code で開くようにしてください。
...
- サンプル本文と既定の校正方針は `，` と `．` を前提にしています。句読点を変えたい場合は `.textlintrc.js` の `periodMark` や `commaMark` を調整してください
