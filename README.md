...
このテンプレートから作成した自分のリポジトリを開いてコンテナに入り、`main.tex` を保存すれば、そのまま `out/` に PDF が出力されます。

初期状態では LuaLaTeX を前提にしています。`textlint`、`chktex`、`latexindent`、PDF プレビュー、各種スニペットもあわせて使えるようにしてあります。

## 想定している使い方

- Docker と VS Code が入った PC で使う
...
- GitHub アカウント
  `Use this template` で自分用リポジトリを作る場合に使います。

作成したリポジトリのフォルダを VS Code で開くと、`.vscode/extensions.json` に基づいて推奨拡張の案内が表示されることがあります。
すでにインストール済みなら特に何も出ないこともあります。
ホスト側で必須なのは `Dev Containers` です。LaTeX 関連の拡張はコンテナ側に入ります。

### 初回起動

まず GitHub 上で自分用のリポジトリを作ります。
通常は `Use this template` を使うのが扱いやすく、upstream との関係を残したい場合だけ fork を使えば十分です。
...
このテンプレート自体を改善したい場合は、テンプレート元のリポジトリをそのままクローンして作業しても構いません。
...
LaTeX ファイルでプレフィックスを入力し、補完候補を確定するか `Tab` を押すと展開できます。
定義本体は [`.vscode/latex.code-snippets`](./.vscode/latex.code-snippets) にあります。
一覧と補足を日本語でまとめたものは [docs/snippets.md](./docs/snippets.md) に置いてあります。
...
- `subfig`: `subfigure` 環境
- `tbl`: 基本的な `table`
- `btab`: `booktabs` を使う `table`
- `tab`: `tabular`
- `mini`: `minipage`
- `img`: `\includegraphics{...}`

`fig`、`tbl`、`btab` は、まずソースの順に図表を確認しやすいよう `[H]` を使う形にしてあります。
見た目よりページ全体のレイアウトを優先したい場合は、展開後に `[htbp]` などへ変更してください。
...
`main.tex` には `theorem`、`lemma`、`proposition`、`corollary`、`definition`、`example`、`remark` を既定で入れています。
さらに `claim`、`conjecture`、`assumption`、`problem`、`notation` はコメントアウトした形で添えてあるため、そのまま有効化して使えます。

### ひな型

- `jpdoc`: 日本語文書の最小テンプレート
- `childtex`: `% !TeX root = ../main.tex` 付きの子ファイルひな型
